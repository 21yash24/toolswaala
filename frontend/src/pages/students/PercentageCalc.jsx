    ...
    <div>
      <div style={cs}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
          ...
          <div><label style={{ fontSize: 13, color: BRAND.textSecondary, display: "block", marginBottom: 6 }}>Remaining Max Marks</label><input type="number" min="0" value={remaining} onChange={e => setRemaining(+e.target.value || 0)} style={{ width: "100%", padding: 12, borderRadius: 10, border: `1px solid ${BRAND.border}`, background: "rgba(255,255,255,0.03)", color: "white", fontSize: 16 }} /></div>
          <div><label style={{ fontSize: 13, color: BRAND.textSecondary, display: "block", marginBottom: 6 }}>Target % Overall</label><input type="number" min="0" max="100" value={target} onChange={e => setTarget(+e.target.value || 0)} style={{ width: "100%", padding: 12, borderRadius: 10, border: `1px solid ${BRAND.border}`, background: "rgba(255,255,255,0.03)", color: "white", fontSize: 16 }} /></div>
        </div>
      </div>
      <div style={{ ...cs, marginTop: 24, textAlign: "center" }}>
        ...
        {subjects.map((sub, i) => {
          ...
          return (
            <div key={i} style={{ display: "grid", gridTemplateColumns: "1fr 100px 60px", gap: 12, marginBottom: 12, alignItems: "center" }}>
              <input value={sub.name} onChange={e => { const n = [...subjects]; n[i].name = e.target.value; setSubjects(n); }} style={{ padding: 10, borderRadius: 8, border: `1px solid ${BRAND.border}`, background: "rgba(255,255,255,0.03)", color: "white", fontSize: 14 }} />
              <input type="number" min="0" max="100" value={sub.marks} onChange={e => update(i, +e.target.value || 0)} style={{ padding: 10, borderRadius: 8, border: `1px solid ${BRAND.border}`, background: "rgba(255,255,255,0.03)", color: "white", fontSize: 14, textAlign: "center" }} />
