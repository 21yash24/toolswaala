  ...
  return (
    <div className="fade-in">
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(300px, 1fr))", gap: 24 }}>
        <div style={cs}>
          <h3 style={{ color: "white", marginBottom: 20 }}>Exam Countdown ⏱️</h3>
          <div style={{ display: "flex", gap: 10, marginBottom: 24 }}>
            <input value={newExam.name} onChange={e => setNewExam({ ...newExam, name: e.target.value })} placeholder="Exam Name" style={{ flex: 1, padding: 10, borderRadius: 8, border: `1px solid ${BRAND.border}`, background: "rgba(255,255,255,0.03)", color: "white", fontSize: 14 }} />
            <input type="date" value={newExam.date} onChange={e => setNewExam({ ...newExam, date: e.target.value })} style={{ width: 130, padding: 10, borderRadius: 8, border: `1px solid ${BRAND.border}`, background: "rgba(255,255,255,0.03)", color: "white", fontSize: 14 }} />
            ...
            {exams.map(e => {
              ...
              return (
                <div key={e.id} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: 16, borderRadius: 12, background: "rgba(255,255,255,0.02)", border: `1px solid ${BRAND.border}` }}>
                  <div>
                    <div style={{ fontWeight: 600, color: "white" }}>{e.name}</div>
        ...
        <div style={cs}>
          <h3 style={{ color: "white", marginBottom: 20 }}>Study Checklist ✅</h3>
          <div style={{ display: "flex", gap: 10, marginBottom: 24 }}>
            <input value={newTask} onChange={e => setNewTask(e.target.value)} onKeyDown={e => e.key === "Enter" && addTask()} placeholder="Add a topic to study..." style={{ flex: 1, padding: 10, borderRadius: 8, border: `1px solid ${BRAND.border}`, background: "rgba(255,255,255,0.03)", color: "white", fontSize: 14 }} />
          ...
          <div style={{ display: "grid", gap: 8 }}>
            {tasks.map(t => (
              <div key={t.id} style={{ display: "flex", alignItems: "center", gap: 12, padding: "12px 16px", borderRadius: 10, background: t.done ? "transparent" : "rgba(255,255,255,0.03)", border: `1px solid ${BRAND.border}`, opacity: t.done ? 0.5 : 1 }}>
                ...
                <span style={{ flex: 1, fontSize: 14, color: "white", textDecoration: t.done ? "line-through" : "none" }}>{t.text}</span>
      ...
      <div style={{ ...cs, marginTop: 24, textAlign: "center", background: "linear-gradient(90deg, rgba(124,58,237,0.1) 0%, rgba(239,68,68,0.1) 100%)" }}>
        ...
        <p style={{ margin: 0, color: "white", fontWeight: 600 }}>"The best way to predict your future is to create it." — Abraham Lincoln</p>
      </div>
    </div>
  );
}
