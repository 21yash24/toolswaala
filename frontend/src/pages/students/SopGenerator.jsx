      ...
      <div style={cs}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
          <div style={{ gridColumn: "span 2" }}>
            ...
            <select value={form.purpose} onChange={e => update("purpose", e.target.value)} style={{ width: "100%", padding: 12, borderRadius: 10, border: `1px solid ${BRAND.border}`, background: BRAND.surfaceCard, color: BRAND.text }}>
          ...
          <div>
            ...
            <input value={form.field} onChange={e => update("field", e.target.value)} placeholder="e.g. Data Science" style={{ width: "100%", padding: 12, borderRadius: 10, border: `1px solid ${BRAND.border}`, background: "rgba(0,0,0,0.02)", color: BRAND.text }} />
          </div>
          <div>
            <label style={{ fontSize: 13, color: BRAND.textSecondary, display: "block", marginBottom: 6 }}>Target University (Optional)</label>
            <input value={form.university} onChange={e => update("university", e.target.value)} placeholder="e.g. Stanford University" style={{ width: "100%", padding: 12, borderRadius: 10, border: `1px solid ${BRAND.border}`, background: "rgba(0,0,0,0.02)", color: BRAND.text }} />
          </div>
          <div>
            <label style={{ fontSize: 13, color: BRAND.textSecondary, display: "block", marginBottom: 6 }}>Your Current Degree</label>
            <input value={form.degree} onChange={e => update("degree", e.target.value)} placeholder="e.g. B.Tech CS" style={{ width: "100%", padding: 12, borderRadius: 10, border: `1px solid ${BRAND.border}`, background: "rgba(0,0,0,0.02)", color: BRAND.text }} />
          </div>
          <div>
            <label style={{ fontSize: 13, color: BRAND.textSecondary, display: "block", marginBottom: 6 }}>Current CGPA / %</label>
            <input value={form.cgpa} onChange={e => update("cgpa", e.target.value)} placeholder="e.g. 8.5" style={{ width: "100%", padding: 12, borderRadius: 10, border: `1px solid ${BRAND.border}`, background: "rgba(0,0,0,0.02)", color: BRAND.text }} />
          </div>
          <div style={{ gridColumn: "span 2" }}>
            <label style={{ fontSize: 13, color: BRAND.textSecondary, display: "block", marginBottom: 6 }}>Key Achievements (Comma separated)</label>
            <textarea value={form.achievements} onChange={e => update("achievements", e.target.value)} placeholder="e.g. Published a paper in IEEE, Won National Hackathon" style={{ width: "100%", padding: 12, borderRadius: 10, border: `1px solid ${BRAND.border}`, background: "rgba(0,0,0,0.02)", color: BRAND.text, minHeight: 60 }} />
          </div>
          <div style={{ gridColumn: "span 2" }}>
            <label style={{ fontSize: 13, color: BRAND.textSecondary, display: "block", marginBottom: 6 }}>Short Term Career Goal</label>
            <input value={form.careerGoal} onChange={e => update("careerGoal", e.target.value)} placeholder="e.g. To work as an AI Researcher" style={{ width: "100%", padding: 12, borderRadius: 10, border: `1px solid ${BRAND.border}`, background: "rgba(0,0,0,0.02)", color: BRAND.text }} />
          </div>
        </div>
      ...
      {sop && (
        <div style={{ ...cs, marginTop: 32 }} className="fade-in">
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
            <h3 style={{ color: BRAND.text, margin: 0 }}>Your Generated SOP</h3>
            <button onClick={() => { navigator.clipboard.writeText(sop); alert("Copied!"); }} style={{ padding: "8px 16px", borderRadius: 8, border: "none", background: "rgba(255,255,255,0.1)", color: BRAND.text, cursor: "pointer", fontSize: 13 }}>📋 Copy Text</button>
          </div>
          <textarea value={sop} readOnly style={{ width: "100%", padding: 20, borderRadius: 12, border: `1px solid ${BRAND.border}`, background: "rgba(0,0,0,0.2)", color: BRAND.text, minHeight: 400, fontSize: 15, lineHeight: 1.6, fontFamily: "serif" }} />
