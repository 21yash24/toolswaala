...
export default function CgpaCalculator({ initialUniIdx }) {
  const [tab, setTab] = useState(() => initialUniIdx !== undefined ? 2 : parseInt(typeof window !== "undefined" ? localStorage.getItem("tw_cgpa_tab") || "0" : "0", 10));
  ...
  return (
    <div>
      ...
      {tab === 0 && <SgpaTab />}
      {tab === 1 && <CgpaTab />}
      {tab === 2 && <ConvertTab initialUniIdx={initialUniIdx} />}
      {tab === 3 && <TargetTab />}
...
function ConvertTab({ initialUniIdx }) {
  const [cgpa, setCgpa] = useState(() => parseFloat(typeof window !== "undefined" ? localStorage.getItem("tw_cgpa_val") || "8.0" : "8.0"));
  const [uniIdx, setUniIdx] = useState(() => initialUniIdx !== undefined ? initialUniIdx : parseInt(typeof window !== "undefined" ? localStorage.getItem("tw_cgpa_uni") || "0" : "0", 10));
  const [reverseMode, setReverseMode] = useState(() => typeof window !== "undefined" && localStorage.getItem("tw_cgpa_rev") === "true");
  const [pct, setPct] = useState(() => parseFloat(typeof window !== "undefined" ? localStorage.getItem("tw_cgpa_pct") || "76" : "76"));
