...
function fix(file) {
  ...
  code = code.replace(/\$\{window\.location\.href\}/g, "${typeof window !== 'undefined' ? window.location.href : 'https://toolswaala.in'}");
  
  // also fix window.location.href outside template strings
  code = code.replace(/url\:\s*window\.location\.href/g, "url: typeof window !== 'undefined' ? window.location.href : 'https://toolswaala.in'");
